...
const emptyDraft = (): DraftState => ({
  ...
  heroLayout: "full", heroBackground: "", coverMediaId: undefined,
});
...
export default function ProjectsPage() {
  ...
  // Fetch full project record (with images + full content) when editing existing
  const projectQuery = useProject(editingId && editingId !== "new" ? editingId : null);

  // Re-hydrate draft from full single-project record once it loads
  useEffect(() => {
    if (!projectQuery.data || contentHydrated) return;
  ...
  }, [projectQuery.data, contentHydrated]);
          ...
          <Card key={p.id} className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                variant="secondary"
                onClick={() => {
                  setDraft({ ...emptyDraft(), ...p, coverMediaId: undefined });
                  setContentBlocks((p.content as ArticleBlock[] | undefined) ?? [{ type: "p", text: "" }]);
                  setContentHydrated(false);
                  setEditingId(p.id);
                }}
